...
description: >
  ...
  wider results. Keyless web-search CLI that queries the Exa, Parallel,
  Tavily, and Firecrawl AI search MCP servers in parallel and returns
...
Searches the web without API keys by POSTing JSON-RPC `tools/call` requests to the keyless MCP endpoints of Exa (`https://mcp.exa.ai/mcp`), Parallel (`https://search.parallel.ai/mcp`), Tavily (`https://mcp.tavily.com/mcp/` with the `X-Tavily-Access-Mode: keyless` header), and Firecrawl (`https://mcp.firecrawl.dev/v2/mcp`). All four providers always run in parallel; one failing (e.g. rate-limited) never blocks the others.
...
With 2+ queries, `--json` emits an array of `{ query, exa, parallel, tavily, firecrawl }` objects instead.
...
- Keyless endpoints share public rate limits; Tavily and Firecrawl report their spent budget as an HTTP 200 error mentioning the "monthly keyless limit" and Exa/Parallel use HTTP 429. Firecrawl's free tier is 1,000 searches/month shared per IP. Failed providers appear under `## Provider errors`; retry later or rely on the others.
